...
mod tests {
    ...
    fn test(tree: &[Option<i32>], expected: &[Option<i32>]) {
        ...
        let result = Solution::lca_deepest_leaves(input);
    ...
    #[test]
    fn my_extreme_ex1() {
        // Tree that's just all left children for all 1000 nodes.
        let mut input = vec![Some(1)];
        input.extend((2..=1000).map(|i| [Some(i), None]).flatten());
        let expected = [Some(1000)];
        test(&input, &expected);
    }

    #[test]
    fn my_extreme_ex2() {
        // Tree that's just all right children for all 1000 nodes.
        let mut input = vec![Some(1)];
        input.extend((1..=1000).map(|i| [None, Some(i)]).flatten());
        let expected = [Some(1000)];
        test(&input, &expected);
    }
