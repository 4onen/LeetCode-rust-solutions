...
// Initial binary heap sol'n
// impl Solution {
//     pub fn furthest_building(heights: Vec<i32>, mut bricks: i32, mut ladders: i32) -> i32 {
//         assert!(heights.len() >= 1);
//         assert!(bricks >= 0);
//         assert!(ladders >= 0);
//         let mut heap = std::collections::BinaryHeap::with_capacity(ladders as usize);
//         for i in 0..heights.len() - 1 {
//             let diff = heights[i + 1] - heights[i];
//             if diff > 0 {
//                 if ladders > 0 {
//                     heap.push(std::cmp::Reverse(diff));
//                     ladders -= 1;
...
impl Solution {
    pub fn furthest_building(heights: Vec<i32>, bricks: i32, ladders: i32) -> i32 {
        ...
        assert!(bricks >= 0);
        assert!(ladders >= 0);
        assert!(ladders <= heights.len() as i32);
        let n = heights.len() as u32 - 1;
        let mut bricks = bricks as u32;
        let mut ladders = ladders as u32;
        let mut heap = std::collections::BinaryHeap::with_capacity(ladders as usize);
        let mut i: u32 = 0;
        while i < n && ladders > 0 {
            let diff = heights[(i + 1) as usize] - heights[i as usize];
            if diff > 0 {
        ...
        while i < n {
            ...
            if diff > 0 {
                let diff = diff as u32;
                match heap.peek_mut().as_deref_mut() {
                    Some(std::cmp::Reverse(top)) if *top < diff && *top <= bricks => {
                        bricks -= *top;
                        *top = diff;
                    }
                    _ if diff <= bricks => bricks -= diff,
                    _ => return i as i32,
                }
            }
            i += 1;
...
mod tests {
    ...
    fn discussion_case1() {
        assert_eq!(
            Solution::furthest_building(vec![1, 5, 1, 2, 3, 4, 10000], 4, 1),
            5
        );
    }
    ...
    fn discussion_case3() {
        assert_eq!(
            Solution::furthest_building(
                vec![
                    40000, 39999, 40001, 39999, 40001, 39999, 40001, 39998, 40002, 39996, 40002,
                    39995, 40002, 39995, 40003, 39995, 40003, 39994, 40004, 39994, 40004
                ],
                0,
                0,
            ),
    ...
    fn discussion_case4() {
        assert_eq!(
            Solution::furthest_building(
                vec![
                    50000, 1000000, 50286, 937590, 149237, 922047, 225201, 919442, 347953, 845985,
                    360602, 827333, 431540, 766505, 443857, 753764, 485558, 719293, 527161, 628036,
                    549150
                ],
                21050,
                9,
    ...
    #[test]
    fn failing_case1() {
        assert_eq!(Solution::furthest_building(vec![1], 0, 0), 0)
    }
}
